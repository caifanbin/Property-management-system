...
| Updated: 17.04.2003                               |
 updated: 2007-8-27 by zcy checkbox
|--------------------------------------------------*/
...
function dTree(imgRootpath,objName) {
	...
	this.icon = {
		...
		root				: imgRootpath+'/img/base.gif',

		folder			: imgRootpath+'/img/folder.gif',

		folderOpen	: imgRootpath+'/img/folderopen.gif',

		node				: imgRootpath+'/img/page.gif',

		empty				: imgRootpath+'/img/empty.gif',

		line				: imgRootpath+'/img/line.gif',

		join				: imgRootpath+'/img/join.gif',

		joinBottom	: imgRootpath+'/img/joinbottom.gif',

		plus				: imgRootpath+'/img/plus.gif',

		plusBottom	: imgRootpath+'/img/plusbottom.gif',

		minus				: imgRootpath+'/img/minus.gif',

		minusBottom	: imgRootpath+'/img/minusbottom.gif',

		nlPlus			: imgRootpath+'/img/nolines_plus.gif',

		nlMinus			: imgRootpath+'/img/nolines_minus.gif'
...
dTree.prototype.toString = function() {
	...
	var str = '<div class="dtree"><form id="'+this.config.checkBoxName+'">\n';//add dTreeCheckBox
...
dTree.prototype.addNode = function(pNode) {
	...
	for (n; n<this.aNodes.length; n++) {
		if (this.aNodes[n].pid == pNode.id) {
			...
			if(this.aNodes[n].pid == 0){//add parEnum
				this.parEnum = "";
			}
...
dTree.prototype.node = function(node, nodeId) {
	...
	if(this.config.useCheckBox && nodeId!=0){//add
		if(node.pid==0){
		...
		str += '<input type="checkbox" name="checkboxname" id="c'+this.parEnum+'" onClick="caBox(\'c'+this.parEnum+'\',\''+this.config.checkBoxName+'\')" value="' +  node.id+'" class="cx"/>';
	}
...
if (!Array.prototype.pop) {
	...
	Array.prototype.pop = function array_pop() {
		...
		/* checkbox tree */
		function caBox( regx,checkBoxName ) {
			if( document.getElementById( regx ).checked ){
				var form = document.getElementById(checkBoxName);
