...
with (document) {
	...
	write("<B ID=\"CAL_B_BLANK\" style='color:" + gcBG + "; visibility:visible; cursor:hand; font-size:12px' onclick='fSetDate(0,0,0)' onMouseOver='this.style.color=gcToggle' onMouseOut='this.style.color=gcBG'>\u6e05\u7a7a\u9009\u62e9</B>");
	write("</td><td algin='center'>");
	write("<B style='color:" + gcBG + ";cursor:hand; font-size:12px' onclick='fSetDate(giYear,giMonth,giDay)' onMouseOver='this.style.color=gcToggle' onMouseOut='this.style.color=gcBG'>\u4eca\u5929: " + giYear + "-" + giMonth + "-" + giDay + "</B>");
	write("</td></tr></table>");
	write("</TD></TR>");
	write("</TABLE></Div>");
}
function fPopUpDlg(endtarget, ctl, WINname, WINwidth, WINheight) {
	showx = (event.screenX - WINwidth) / 2;
	showy = event.screenY - event.offsetY + 18; // + deltaY;
	newWINwidth = WINwidth + 4 + 18;
	var features = "dialogWidth:" + newWINwidth + "px;" + "dialogHeight:" + WINheight + "px;" + "dialogLeft:" + showx + "px;" + "dialogTop:" + showy + "px;" + "directories:no; localtion:no; menubar:no; status=no; toolbar=no;scrollbars:yes;Resizeable=no";
